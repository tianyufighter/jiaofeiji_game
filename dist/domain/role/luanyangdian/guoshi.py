...
class Guo(pygame.sprite.Sprite):
    """
    皇上
    """
    def __init__(self, x, y):
        self.walk = DirAction("farmer71", "1822-bb2ecf8e-", 4, 8, True)
        self.pos_x = x
        self.pos_y = y
        self.width = 70
        self.height = 81
    ...
    def __move__(self):
        ...
        if self.dir == 0:
            ...
            if self.pos_x < 0 or self.pos_x > (3824 - self.width):
                self.pos_x -= self.step
            self.pos_y += self.step
            if self.pos_y > 0 or self.pos_y > (2400 - self.height):
                self.pos_y -= self.step
        elif self.dir == 1:
            self.pos_x -= self.step
            if self.pos_x < 0 or self.pos_x > (3824 - self.width):
            ...
            if self.pos_x < 0 or self.pos_x > (3824 - self.width):
                self.pos_x -= self.step
            self.pos_y -= self.step
            if self.pos_y > 0 or self.pos_y > (2400 - self.width):
                self.pos_y += self.step
